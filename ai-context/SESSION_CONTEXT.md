...
- Прод: Docker на VPS `37.1.196.83`. Deploy: `deploy.ps1` (Windows) / `deploy.sh` (Linux/macOS)
- Dashboard: **`https://catalystparser.io`** (TLS via Let's Encrypt R13, через nginx reverse-proxy на Docker `127.0.0.1:8080`. `www.catalystparser.io` тоже валиден, 80→443 редирект). Юзеры через `X-API-Key`
- Admin: `127.0.0.1:8081` (loopback-only внутри VPS, через SSH-tunnel, `X-Admin-Key`)
- API auth-by-query (`?apiKey=`, `?key=`) — отключён, только header
- **Голый IP не отвечает**: Docker port `8080` биндится на `127.0.0.1:8080`, единственный путь снаружи — TLS через nginx. ufw разрешает только 22/80/443

## Бизнес-правила

| План | Цена | Длительность | Источники | Manual Analyze | Catalyst forecast | Alerts/day |
|---|---|---|---|---|---|---|
| `free` | $0 | бессрочно | Reddit + Google Trends | 🔒 заблокировано | 🔒 заблокировано | ∞ |
| `test` | $5 | 1 день, one-time | все 5 | 5/день | 5/день | ∞ |
| `pro` | $100 | 30 дней | все 5 | 100/день | 100/день | ∞ |
| `admin` | $0 | бессрочно | все 5 | ∞ | ∞ | ∞ |

**Single source of truth для прав плана** — `src/billing/entitlements.js`. `getPlanEntitlements(planName)` возвращает `{sources: string[], manualAnalyze: number, catalyst: number}`. Caps semantics: -1 = unlimited (admin), 0 = blocked (free), N>0 = N/day soft-cap (in-memory rolling 24h). Импортируется и в dashboard/server.js, и в telegram.js — гарантирует одинаковые правила на сайте и в боте.

- **Daily cap на алерты убран 2026-05-06** — алерты не платная фича, ограничивать смысла нет. Поле `alert_limit` в схеме осталось, но нигде не читается (legacy).
- **15-min cooldown на Catalyst forecast убран 2026-05-06** — Catalyst дёшев (~$0.05/call), daily cap достаточно.
- **30-sec cooldown на manual-analyze оставлен** — anti-dupe, anti-spam (для Pro тоже).
...
| weightMemePotential | **0.45** | 0.45 | 0.45 | **0.40** | 0.10 |
| weightEmergence | 0.20 | 0.10 | 0.10 | 0.20 | **0.35** |
| weightVirality | 0.20 | 0.20 | 0.25 | 0.25 | **0.30** |
...
- **memePotential-доминанта** (2026-05-06): после inversion-bug'а где meme=91 проваливал floor=60 (general meme-вес был 0.30, virality=20 в knock-on'е тащил вниз), бамп общей калибровки — DEFAULT_ALERT_WEIGHTS теперь meme=0.45, general/celebrities соответственно подняты. AI-вердикт по memePotential остаётся primary signal; virality/emergence — модификаторы.
...
Pro/Admin only — постоянное сохранение трендов «на память». Free/Test видят locked-визуал (⭐-кнопка с 🔒, click → upgrade toast).

**Storage** (`user_favorites` table):
- `(chat_id TEXT, trend_id INTEGER, note TEXT, snapshot TEXT, created_at)`, UNIQUE(chat_id, trend_id)
- Без CASCADE / retention sweep — фавориты вечны
- **`snapshot` = JSON-копия** ключевых полей тренда на save-time (title, source, url, image, raw_metrics, alert_type, whyNow, trigger_*, externalId, author). Зачем: trends-таблица может ротировать row'ы (старые тренды удаляются для storage hygiene), но favourite должен пережить. `getFavoritesByChat` — LEFT JOIN: fresh-данные если есть, snapshot fallback когда live-row удалён. `addFavorite` использует `INSERT ... ON CONFLICT DO UPDATE` — повторное сохранение освежает snapshot.
...
- `POST /api/trends/:id/favorite` body опц. `{note}` — добавить
- `DELETE /api/trends/:id/favorite` — убрать
- `PATCH /api/trends/:id/favorite` body `{note}` — изменить заметку
- `GET /api/favorites` — список с merged fresh+snapshot

Все 4 проходят через `_favoriteGate(req, res)` — `getPlanEntitlements(plan).favorites === true`. Иначе 403 reason='plan'.

**Feed integration**:
- `_handleTrends` pre-fetch'ит `Set<favoriteIds>` один раз и передаёт в `_formatTrend(row, userId, favSet)` для O(1) per-row attach `isFavorite`
- `?favoritesOnly=1` фильтр через `WHERE id IN (favoriteIds)` (server-side)
- Возвращает `favoriteCount` в payload — для счётчика в Saved-табе bottom-nav
- `_publicUser.entitlements.favorites: bool` — фронт читает для render-логики

**Frontend**:
- ⭐-кнопка inline в `.feed-user-row` (после avatar, **слева** — отделена от ✕ который absolute справа). Hover-only когда не сохранено, always-visible filled когда saved. Pulse `favPulse` на add. Free/test — кнопка не рендерится.
- ⭐-кнопка в `.modal-head` — **самым левым элементом** (юзер попросил перенести влево от badges/✕).
- `FavoriteNoteEditor` (компонент в SPA) — collapsible block в начале modal-body когда `isFavorite=true`. Cap 500 chars, Cmd/Ctrl+Enter — save.
- Snapshot-banner `🗄 Saved copy — original may have been removed` поверх модалки когда тренд из snapshot.
- Saved-таб в bottom-nav между Feed и Analyze. Toggle-логика: click → `setFavoritesOnly(!favoritesOnly)` + `setOffset(0)` + `view='trends'`. Если на Saved → click повторно = «back to all».
- Optimistic UI в `toggleFavorite` — patch `isFavorite` в trends-list и modalTrend сразу, rollback на 403/500.

**Telegram-бот не интегрирован** (по решению владельца — «бот и так перегружен»). DB-таблица shared, бот может присоседиться позже через callback.
...
- Семантические цвета (`--green/--red/--orange/--yellow/--pink/--purple`) **константны** во всех темах — OK/error сигналы не должны менять оттенок
...
`detectTheme()` дефолтит на `ink`. Юзеры со старой темой (`dim`/`slate`/`mono` или ещё более старые) автоматически переключаются на default через validity-check — миграция не нужна.
...
**Modal sheets** (Settings / Account / Analyze): центрированные с `backdrop-filter: blur(14px)` + затемнением. Body scroll lock. Закрываются по Esc / клик на фон / ✕. (Stats sheet удалён 2026-05-06 — таб убран из bottom-nav, контент дублировал sidebar/right-rail.)

**Bottom nav** (3 таба): **Feed** / **Saved** (⭐, Pro/Admin only) / **Analyze** (🧪, Pro/Admin only). Saved активен когда `view==='trends' && favoritesOnly`, Feed когда `view==='trends' && !favoritesOnly`. Free/Test видят Saved и Analyze с 🔒 + dashed-border, click → upgrade toast. Saved-таб рядом с label показывает счётчик (`favoriteCount` от server). `Inline grid-template-columns: repeat(3, 1fr)`.

**Login screen** (EN-only, не зависит от языка юзера) — переписан 2026-05-06. Лого-кот через `<img src="/assets/logo.png?v=LOGO_VERSION">` с onError-fallback на 🐱 эмоджи. Ambient X-blue gradient blobs + dot-grid mask. Glass-карточка max-width 440px с backdrop-blur. CTA-кнопка плоская (без 3D inset'ов) `var(--accent)` → `#146da8` gradient + SVG paper-plane `rotate(-25deg)` (Telegram-style). Footer — X-pill `𝕏 @Catalystparser` (hoverable). Все цвета через CSS-vars → авто-адаптация под все 4 темы. Tagline «Track narratives across the social web.» (39 chars, single-line, без orphan-слов).

**Top nav (post-auth)**: справа три кнопки — `nav-icon-btn` Telegram-bot link (paper-plane SVG, URL `https://t.me/<BOT_USERNAME>`, username резолвится при старте сервера через `telegram.getBotUsername()` → `bot.getMe()` cached в `this._botUsername`, инжектится в SPA template как `BOT_USERNAME` константа), `nav-icon-btn` X-link `https://x.com/Catalystparser`, account button (avatar или ⚙).

**TrendModal**: head — **⭐ favorite (leftmost)** / alertType / category / 🧪 MANUAL / phase-badge / source / time / ✕. Snapshot-banner `🗄 Saved copy — original may have been removed` появляется поверх модалки если тренд из favourite-snapshot (live-row удалён). Body — **FavoriteNoteEditor** (если saved) → media → title → 🔥 Триггер (whyNow) → 🔮 Каталист (forecast / CTA / **locked-card** с 🔒 для free/test) → links (Source / Telegram / **🧠 Ask Grok**) → 🔥 Топовые твиты (x_trends only) → feedback → score bars → stats grid.
...
**Favourites** (Pro/Admin only) — ⭐-кнопка inline в `.feed-user-row` (после avatar, **слева**) и в `.modal-head` (leftmost, перед всеми badges). Filled когда saved (accent-tint), outline иначе. Hover-only когда не сохранено, always-visible когда saved. Pulse-анимация `favPulse` на add. Поле `note` (cap 500 chars) редактируется в TrendModal через `FavoriteNoteEditor` — collapsible block в начале modal-body с тремя состояниями: «add note» CTA / view + ✏ edit + ✕ remove / textarea + Save/Cancel (Cmd-Enter — save, Esc — cancel). См. отдельную секцию § User favourites.

**Locked-визуалы для Free** (sidebar + Live-панель): premium-источники (Twitter/TikTok/X-Trends) показываются с 🔒-пилюлей вместо счётчика, dim opacity 0.55, dashed-look. В правой Live-панели — pill с dashed-border и серым dot. Click → upgrade-toast `🔒 Этот источник — на Test/Pro`. **`/api/sources` возвращает `inPlan: bool` для каждого источника** на основе `getPlanEntitlements(planName).sources`.

**Catalyst forecast locked-card**: для free/test в TrendModal вместо disabled-кнопки рендерится **информационная locked-карточка** — 36×36 icon-tile с 🔒 на X-blue glow + жирный title + дим subtitle «Available on Test and Pro plans». Не disabled-кнопка, а content-row.
...
| `src/billing/entitlements.js` | Plan entitlements (sources, manualAnalyze, catalyst caps) — shared by bot + dashboard |
...
Catalyst публично захощен на `https://catalystparser.io` (TLS, port lockdown, ufw, daily backup). Все security-блокеры из production-readiness pass (2026-05-04) теперь активированы реально через nginx + Let's Encrypt. Что выставлено:

**Edge stack (на VPS, не в Docker)**:
- **nginx** (`/etc/nginx/sites-available/catalyst`) — reverse-proxy на `127.0.0.1:8080`. SSE-friendly (`proxy_buffering off`, `proxy_read_timeout 24h`). Стандартный набор `Host/X-Real-IP/X-Forwarded-*` headers + Authorization passthrough. `set_real_ip_from 127.0.0.1` + `real_ip_header X-Forwarded-For` чтобы downstream rate-limit видел реальный IP юзера.
- **Let's Encrypt cert** (R13, `/etc/letsencrypt/live/catalystparser.io/`) — auto-renew через `certbot.timer` (systemd, ежедневно). Покрывает `catalystparser.io` + `www.catalystparser.io`, 80→443 редирект.
- **ufw**: default deny incoming, allow только 22 (SSH), 80 (HTTP→HTTPS + cert challenges), 443 (HTTPS). Включается при ребуте VPS.
- **Daily backup** (`/usr/local/bin/catalyst-backup.sh`): cron 03:30 UTC. Discover'ит mountpoint named volume `catalyst_data` через `docker volume inspect`, делает `sqlite3 .backup` (locking-aware, hot snapshot — безопасно при concurrent writes), gzip → `/var/backups/catalyst/catalyst_<date>.db.gz`. Retention 14 дней. Лог в `/var/log/catalyst-backup.log`. **На том же VPS** — off-site copy (S3/Backblaze) пока нет, riskует если VPS целиком умрёт.

**Что выставлено в коде** (production-readiness pass, 2026-05-04):
...
- `NODE_ENV=production` — в production app падает на старте если отсутствует `XAI_API_KEY` / `TELEGRAM_BOT_TOKEN` / `ADMIN_API_KEY`. В dev — warnings. Сейчас прод стоит в production через docker-compose.
- `PUBLIC_BASE_URL=https://catalystparser.io` — заполнен. Канонический origin для absolute links / OG-тегов / Telegram deep-links.
- `TRUST_PROXY=1` — заполнен. App за nginx, rate-limit'ы читают real-IP через `X-Forwarded-For`.
- `DASHBOARD_ALLOWED_ORIGINS=https://catalystparser.io` — заполнен. CORS allowlist (single origin). `ADMIN_ALLOWED_ORIGINS` пуст — same-origin only (админка только через SSH-tunnel).
