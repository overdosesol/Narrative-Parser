...
`src/refresh/hot-metrics.js` — фоновый цикл, который **раз в 12 часов** (env `HOT_REFRESH_INTERVAL_MINUTES`, default **720** — было 120, поднято 2026-05-04 для cut Grok costs 6×) пере-фетчит свежие метрики живых трендов из источника и заново прогоняет их через скоринг. Цель — дать «дозреть» бордерлайн-трендам, которые на момент первого скоринга были чуть ниже Stage 2 порога, но потом набрали виральность.

**По сути — бесконечная петля без per-trend cooldown'а** (picker не читает `lastRefreshedAt`, хотя оно пишется в `raw_metrics`). Возрастной gate `first_seen_at > 24h ago` единственная защита. При интервале 12h тренд за свою жизнь попадёт в refresh **максимум 2 раза**. Если ещё нужно резать расходы — следующий шаг добавить `lastRefreshedAt` cooldown в `getHotTrendsForRefresh` или `HOT_REFRESH_INTERVAL_MINUTES=1440` (раз в сутки = ровно один refresh на тренд).
...
**Стоимость**: ~$0.5/мес (Stage 1 LLM на ~50 трендах × **2 циклов/день** при 12h интервале с gpt-5.4-mini). Stage 2 cap'ом ограничен — общий бюджет ~$153/мес не растёт. Раньше при 2h интервале — ~$3/мес; снижение в 6× после смены DEFAULT_INTERVAL_MIN.
...
- `HOT_REFRESH_INTERVAL_MINUTES` — интервал refresh-цикла (default **720** = 12h, было 120; 1440 = раз в сутки для минимума расходов)
