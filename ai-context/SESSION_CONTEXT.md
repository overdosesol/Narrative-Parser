...
- **Dashboard**: Theme system · Dashboard layout · Sidebar filters
- **Admin panel**
...
- Admin: `127.0.0.1:8081` (loopback-only, через SSH-tunnel, `X-Admin-Key`).
...
- `settings.presetConfigsAuto` — Grok auto-overrides (weekly tag-refresher)
- `settings.presetTagsLocked` — per-tag pin lock-mask `{<preset>: {reddit: [...], twitter: [...]}}`. Locked tags не удаляются при auto-refresh.
...
**Cycle pacing**: TikTok работает независимо от активного пресета (`sources.tiktok.enabled` в preset-config помечено DEPRECATED). Управление через env `TIKTOK_ENABLED`. Свой интервал `TIKTOK_CYCLE_INTERVAL_MINUTES=30` (vs глобальные 15 мин). Time-gate в `collect()` skip'ает если последний run < interval. First-run after restart всегда проходит.
...
Weekly background job — Grok `grok-4.3` (fallback `grok-4.20-0309-reasoning`) предлагает свежие subreddits и Twitter keyword groups для каждого из 5 пресетов. Решает проблему «теги устаревают, надо вручную обновлять раз в 1-2 недели» которая стояла после TikTok meme refresh от 2026-05-05.

**Архитектура** (`src/refresh/tag-refresher.js`):
- Hourly check loop в `index.js` — фоновый, 5 мин после boot, потом каждый час. Проверяет cooldown (default 7 дней через `TAG_REFRESH_COOLDOWN_DAYS`), при истечении — фоновый `refreshAll()`, не блочит scan-cycle.
...
**Login screen** (EN-only). Лого-кот через `<img src="/assets/logo.png?v=LOGO_VERSION">` с onError-fallback на 🐱 эмоджи. Ambient gradient blobs + dot-grid mask. Glass-карточка max-width 440px с backdrop-blur. CTA-кнопка плоская `var(--accent)` → `#146da8` gradient + SVG paper-plane `rotate(-25deg)`. Footer — X-pill `𝕏 @Catalystparser`. Все цвета через CSS-vars → авто-адаптация.
...
| `src/dashboard/server.js` | Dashboard SPA (inline React в template literal) |
...
**Фикс**: helper `sqliteCutoff(msAgo)` в `dashboard/server.js` (~строка 113) форматирует под формат стораджа: `new Date(...).toISOString().slice(0, 19).replace('T', ' ')`. Применён в `_handleTrends`, `_handleStats`, `_handleSources`. **При написании новых date-сравнений с TEXT-колонками используй этот helper, не голый `toISOString()`.**

Тот же баг есть в `db/database.js` (retention/dedup queries), но окна 7-30 дней — симптоматически невидим. Не трогали в первом проходе.
