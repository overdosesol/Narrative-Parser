...
- Любая работа с кодом (написание, рефакторинг, code review, архитектурный анализ) → дефолт sonnet через Agent (без указания модели).
- Поиск в интернете / поиск и чтение файлов по проекту → Agent({model: "haiku", subagent_type: "general-purpose"}). Дёшево и быстро.
