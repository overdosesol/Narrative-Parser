 ...
 * assess the narrative. Returns null when the trend is missing the bits
 * needed to form a useful prompt.
 *
 * Prompt structure mirrors the dashboard's Ask Grok modal-link prompt
 * (`src/dashboard/server.js` Ask-Grok section) \u2014 keep the two in sync.
 * 6-point breakdown: name (find, don't invent) / why viral / why grow /
 * potential / risks / audience. grok.com accepts `?q=<url-encoded-text>`
 * and auto-sends it as the first message.
 */
function buildGrokUrl(trend, lang = 'en') {
  ...
  const sourceLine = url
    ? (lang === 'ru' ? '\u0418\u0441\u0442\u043E\u0447\u043D\u0438\u043A: ' + url : 'Source: ' + url)
    : '';
